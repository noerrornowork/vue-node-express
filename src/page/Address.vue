  ...
  <div>
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item, index) in addressListFilter" :key="index" :class="{'check': checkIndex===index}" @click="checkIndex=index;selectedAddressId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-set-default">
                  <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click.stop="setDefault(item.addressId)"><i>Set default</i></a>
                </div>
        ...
        <div class="shipping-method-wrap">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path: '/orderConfirm', query: {addressId: selectedAddressId}}" tag="a">Next</router-link>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-transfer-dom>
      <x-dialog v-model="show" class="dialog-demo" hide-on-blur>
        ...
        <div class="btn-group">
          <button @click="delAddress" class="confirmBtn">确认</button>
...
<script>
  ...
  import { getAddressList, setDefaulyByAddessId, delAddessByAressId } from '@/api'
  export default {
    created () {
      this.init()
    },
    ...
    methods: {
      ...
      async setDefault (addressId) {
        let res = await setDefaulyByAddessId(addressId)
        console.log(res)
        if (res.status === '0') {
          this.init()
        }
      ...
      async delAddress () {
        console.log('%%%%%%%')
        let res = await delAddessByAressId(this.addressId)
        console.log(res)
        if (res.status === '0') {
          this.show = false
          this.init()
